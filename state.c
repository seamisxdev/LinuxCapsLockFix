...
/*CAPS LOCK DISENGAGE ON PRESS AGAIN FIX */
static void
xkb_filter_mod_lock_new(struct xkb_state *state, struct xkb_filter *filter)
{
    xkb_mod_mask_t caps_mask = (UINT32_C(1) << XKB_MOD_INDEX_CAPS);

    filter->priv = (state->components.locked_mods &
                    filter->action.mods.mods.mask);

    // Special handling for Caps Lock: toggle on press instead of waiting for release, that is part of our Caps Locks disengage on press again fix !
    if ((filter->action.mods.mods.mask & caps_mask) &&
        !(filter->action.mods.flags & ACTION_LOCK_NO_LOCK)) {
        // Toggle the lock state immediately on press
        state->components.locked_mods ^= caps_mask;
        // Don't set base mods for Caps Lock
        state->set_mods &= ~caps_mask;
        // Mark as processed
        filter->priv = state->components.locked_mods & caps_mask;
        }
    else if (filter->priv && (filter->action.mods.flags & ACTION_UNLOCK_ON_PRESS)) {
        /*
                         ...
                         enum xkb_key_direction direction)
{
    ...
    // Special handling for Caps Lock: don't clear anything on release
    if (filter->action.mods.mods.mask & caps_mask) {
        // Lock was already toggled on press, just clean up the filter
        filter->func = NULL;
        return XKB_FILTER_CONTINUE;
    }

    // Normal behavior for other lock modifiers
...
xkb_state_update_derived(struct xkb_state *state)
{
    ...
    /* * THE FIX: If Caps Lock is logically LOCKED,
     * we tell the app it is also physically DEPRESSED (base_mods).
     */
    if (state->components.locked_mods & caps_mask) {
        state->components.base_mods |= caps_mask;
    }
                     ...
                     enum xkb_key_direction direction)
{
    ...
    if (xkb_state_key_get_one_sym(state, kc) == XKB_KEY_Caps_Lock) {
        if (direction == XKB_KEY_DOWN) {
            state->components.locked_mods ^= caps_mask;
        }
        /* We let update_derived handle the base_mods sync */
                         ...
                         enum xkb_state_component type)
{
    ...
    if (type & XKB_STATE_MODS_DEPRESSED)
        ret |= (state->components.base_mods & ~caps_mask);
